...
<script>
  ...
  import { BasicModal, useModalInner } from '@/components/comps/Modal';
  //   import { BasicTable, useTable } from '@/config/components/Table';
  // hooks
  ...
  import { buildUUID } from '@/config/utils/uuid';
    ...
    setup(props, { emit }) {
      ...
      function beforeUpload(file) {
        ...
        if (maxSize && file.size / 1024 / 1024 >= maxSize) {
          createMessage.error(`只能上传不超过${maxSize}的文件!`);
          return false;
        }

        // 设置类型,则判断
        if (accept.length > 0 && !checkFileType(file, accept)) {
          createMessage.error(`只能上传${accept.join(',')}的类型文件!`);
          return false;
        }
        const commonItem = {
          uuid: buildUUID(),
      ...
      function handleOk() {
        ...
        if (isUploadingRef.value) {
          return createMessage.warning('saveWarn');
        }
        const fileList = [];
        ...
        if (fileList.length <= 0) {
          return createMessage.warning('saveError');
        }
        fileListRef.value = [];
        closeModal();
      ...
      function handleCloseFunc() {
        if (!isUploadingRef.value) {
          ...
          createMessage.warning('uploadWait');
          return false;
        }
      }
